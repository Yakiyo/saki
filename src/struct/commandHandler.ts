    ...
    public async handleCommand(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            if (interaction.replied || interaction.deferred) {
                ...
                await interaction.reply({ content: 'Internal error when executing the command!', ephemeral: true });
            }
        }
    }
    ...
    private async registerCommands() {
        ...
        for (const folder of folders) {
            ...
            for (const file of files) {
                const { command } = require(resolve(process.cwd(), `${path}/${folder}/${file}`)) as { command: Command };
