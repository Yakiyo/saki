...
export const command: Command = {
    ...
    async execute(interaction) {
        ...
        const invite = await interaction.client.fetchInvite(interaction.options.getString('invite') as string).catch(() => null);
        if (!invite) {
        ...
        const channel = await interaction.client.channels.fetch(c.channels.affiliate)
            .then(c => c)
            .catch(() => null);
        if (!channel) {
