...
export const command: Command = {
    ...
    async execute(interaction) {
        ...
        if (!/^https?:\/\/.*\/.*\.(png|gif|webp|jpeg|jpg)\??.*$/gmi.test(attachment)) return interaction.reply({ content: 'Second argument does not seem to be a valid image link. Please provide a link to a png/jpg/webp/gif file.', ephemeral: true });
