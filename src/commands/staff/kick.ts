...
export const command: Command = {
    ...
    async execute(interaction) {
        ...
        const target = await interaction.guild?.members.fetch(interaction.options.getUser('user')?.id as string) as GuildMember;
        ...
        if (!target.kickable) {
            interaction.editReply('Target user is higher than me in hierarchy. Cannot kick them. Please give me a higher role');
            return;
        }
        await target.send({
            embeds: [{
