import { SlashCommandBuilder, ChannelType as CT, GuildTextBasedChannel, Colors, APIEmbed } from "discord.js";
...
export const command: Command = {
	...
	async execute(interaction) {
        ...
        if (subCommand === 'source') {
            ...
            const message = await (channel as GuildTextBasedChannel).messages.fetch(`${msgid}`).then(res => res).catch(() => undefined);
            if (!message) {
            ...
            if (json.match(/https:\/\/sourceb\.in\/.*|https:\/\/srcb\.in\/.*/g)) {
                const result = await sourcebin.get(sourcebin.url(json))
                    .then(res => (res.files[0].content))
                    .catch(() => null);
                if (!result) {
                    interaction.editReply('Could not fetch the provided sourcebin URL. Please make sure it\'s a valid link and has some JSON content in it');
                    return;
                }
                try {
            ...
            try {
                ...
                interaction.editReply('Could not send embed due to unexpected errors. Possible erros: Invalid JSON expression or breaking [discord embed limitations](https://discord.com/developers/docs/resources/channel#embed-limits \'Discord Embed Limits\')');
                return;
            }
            ...
            const message = await (channel as GuildTextBasedChannel).messages.fetch(messageId as string).then(res => res).catch(() => null);
            ...
            try {
                ...
                log(error);
                interaction.editReply('Unexpected error while editing message. Possible causes: Bot doesnt have permission to view channel, invalid JSON value, internal discord error');
                return;
            }
        }
